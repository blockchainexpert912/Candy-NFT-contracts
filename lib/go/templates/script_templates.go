...
func GenerateBorrowNFTScript(nftAddress, CandyNFTAddress flow.Address) []byte {
...
func GenerateGetNFTMetadataScript(nftAddress, CandyNFTAddress, metadataAddress flow.Address) []byte {
	code := assets.MustAssetString(filenameGetNFTMetadata)
	return replaceAddresses(code, nftAddress, CandyNFTAddress, metadataAddress, flow.EmptyAddress)
}

// GenerateGetNFTViewScript creates a script that returns the rollup NFT View for an NFT.
func GenerateGetNFTViewScript(nftAddress, CandyNFTAddress, metadataAddress flow.Address) []byte {
...
// from storage and tries to borrow a reference for an NFT that it owns.
// If it owns it, it will not fail.
func GenerateGetCollectionLengthScript(nftAddress, CandyNFTAddress flow.Address) []byte {
...
func GenerateGetTotalSupplyScript(nftAddress, CandyNFTAddress flow.Address) []byte {
