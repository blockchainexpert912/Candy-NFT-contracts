...
describe("NonFungibleToken Contract Tests", () => {
    ...
    // Deploy Candy NFT contract and verify deployment with
    ...
    test("Should have properly initialized fields after deployment", async () => {
        ...
        const { _1, CandyNFTAccount, _2, contractParams } = await getTestAddressesAndContractParams();
        ...
        await assertCollectionLength(CandyNFTAccount, expectedCollectionLength);
    });

    // Deploy Candy NFT contract and mint a token
    test("Should be able to mint a token", async () => {
        ...
        const expectedCollectionPath = "/public/CandyNFTCollection";

        // Deploy all contracts
        const { nftAccount, CandyNFTAccount, _, contractParams } = await getTestAddressesAndContractParams();
        await deployContracts(contractParams);
        ...
        await mintNFT(CandyNFTAccount, nftAccount);
    ...
    test("Shouldn't be able to withdraw an NFT that doesn't exist in a collection", async () => {
        ...
        const testNFTID = 0;
        // Deploy all contracts
        const { _, CandyNFTAccount, joshAccount, contractParams } = await getTestAddressesAndContractParams();
        await deployContracts(contractParams);
    ...
    test("Should be able to withdraw an NFT and deposit to another accounts collection", async () => {
        ...
        // Deploy all contracts
        const { nftAccount, CandyNFTAccount, joshAccount, contractParams } = await getTestAddressesAndContractParams();
        await deployContracts(contractParams);
    ...
    test("Should resolve view with contract borrowing", async() => {
        // Deploy all contracts
        const { CandyNFTAccount, _, contractParams } = await getTestAddressesAndContractParams();
        await deployContracts(contractParams);

        await assertStoragePath(CandyNFTAccount);
    })
});
