...
// Asserts that total supply of CandyNFT matches passed expected total supply
...
export async function assertStoragePath(address) {
    const storagePath = await executeGetContractStoragePath(address, "CandyNFT");
    expect(storagePath.domain).toBe("storage")
    expect(storagePath.identifier).toBe("CandyNFTCollection")
}
